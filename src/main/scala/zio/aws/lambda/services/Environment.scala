...
object Environment extends Serializable {
  ...
  val ENV_LOG_GROUP_STREAM     = "AWS_LAMBDA_LOG_STREAM_NAME"
  ...
  def fromEnvironment: ZIO[System, Error, Environment] =
    ZIO.foreach(all)(k => env(k).map(k -> _)).map(_.toMap).mapError(HttpError(_)).flatMap { vars =>
      ...
      if (missingKeys.nonEmpty)
        ...
        IO.succeed(new Environment {
          val lambdaEnv = new Service[Any] with Blocking.Live {
            ...
            val logGroupStream     = vars(ENV_LOG_GROUP_STREAM).get
